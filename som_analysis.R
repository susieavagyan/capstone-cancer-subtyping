...
# Preprocessing data
altData <- read_csv("./data/mut_cnv_onehot.csv", col_types=c("patient_id"="c"))

altData <- altData[rowSums(altData[-1]) > 1,]
clustData <- read.csv(args$clustfile, sep = "\t")
myData <- merge(clustData, altData )
myData <- altData
indata <- t(as.matrix(myData[, -(1)]))
metadata <- myData[, (1)]
colnames(indata) <- rownames(metadata) <- metadata$patient_id
...
save_dir <- file.path("./results/som")

if (!dir.exists(save_dir)){
  dir.create(save_dir, recursive = TRUE)
} 

env <- opossom.new(list(dataset.name = "./results/som/run01",
...
write.csv(tb_final, "./results/som/propagated_mut.csv", row.names = FALSE)
...
plot(cl$centers, col = cl$cluster, pch = 8, cex = 2)
